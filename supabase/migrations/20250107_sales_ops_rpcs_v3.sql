...
-- V3: Fixed column references and comprehensive function drops
...
-- Drop sales_ops functions (new ones we're creating)
drop function if exists public.sales_ops_category_kpis(date, date) cascade;
drop function if exists public.sales_ops_category_kpis_monthly(date, date) cascade;
drop function if exists public.sales_ops_kpis_by_collection(date, date) cascade;
drop function if exists public.sales_ops_kpis_monthly_by_collection(date, date) cascade;
drop function if exists public.sales_ops_collection_by_dealer(text, date, date) cascade;
drop function if exists public.sales_ops_fill_rate(date, date) cascade;
drop function if exists public.sales_ops_import_lead_time(date, date) cascade;
drop function if exists public.sales_ops_forecast_accuracy(date, date) cascade;
drop function if exists public.sales_ops_inventory_turnover(date, date) cascade;
drop function if exists public.sales_ops_dealer_bounce_rate(date, date) cascade;
drop function if exists public.ops_reports_made_by_month(date, date) cascade;
drop function if exists public.ops_comm_consistency_index(date, date) cascade;
...
create function public.sales_ops_category_kpis(
  from_date date,
  to_date date
)
returns table (
...
as $$
  ...
  select
    ...
    round((a.avg_price - (a.avg_price * 0.60)) / nullif(a.avg_price, 0) * 100, 2) as gross_margin, -- Simulated: 40% margin
...
create function public.sales_ops_category_kpis_monthly(
  from_date date,
  to_date date
)
returns table (
...
create function public.sales_ops_kpis_by_collection(
  from_date date,
  to_date date
)
returns table (
...
create function public.sales_ops_kpis_monthly_by_collection(
  from_date date,
  to_date date
)
returns table (
...
create function public.sales_ops_fill_rate(
  from_date date,
  to_date date
)
returns table (
...
create function public.sales_ops_import_lead_time(
  from_date date,
  to_date date
)
returns table (
...
create function public.sales_ops_forecast_accuracy(
  from_date date,
  to_date date
)
returns table (
...
create function public.sales_ops_inventory_turnover(
  from_date date,
  to_date date
)
returns table (
...
create function public.sales_ops_dealer_bounce_rate(
  from_date date,
  to_date date
)
returns table (
...
create function public.ops_reports_made_by_month(
  from_date date,
  to_date date
)
returns table (
...
create function public.ops_comm_consistency_index(
  from_date date,
  to_date date
)
returns table (
...
as $$
  ...
  -- Note: Only uses columns that exist in loss_opportunities table
  select
    concat('Project-', lo.id) as project_name,
    c.dealer_name as dealer,
    coalesce(lo.sku, 'TBD') as expected_sku, -- Use sku column (not expected_sku)
