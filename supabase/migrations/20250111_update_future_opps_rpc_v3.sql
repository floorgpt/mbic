...
-- Fix SQL joins to work properly without PostgREST relationships
-- Date: 2025-01-11 (v3)
...
AS $$
  ...
  LEFT JOIN public.customers_demo c ON c.customer_id = fso.dealer_id
  LEFT JOIN public.sales_reps_demo r ON r.rep_id = fso.rep_id
...
-- Add comment
COMMENT ON FUNCTION public.list_future_sale_opps_open IS 'Lists ALL future sale opportunities that are unconfirmed by Operations team, regardless of expected close date. Uses LEFT JOIN instead of FK relationships.';
