-- Migration: Add record_id UUID column to loss_opportunities table (v2 - fully idempotent)
...
-- Add unique constraint to record_id (idempotent)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'loss_opportunities_record_id_unique'
  ) THEN
    ALTER TABLE public.loss_opportunities
...
-- Create index for faster lookups by record_id (idempotent)
CREATE INDEX IF NOT EXISTS idx_loss_opportunities_record_id
ON public.loss_opportunities(record_id);

-- Create or replace trigger function to auto-generate record_id for new inserts
